...
import { SpeedUnits, NullWindrose, Windrose } from '../../types/WindRose';
import { parseCsvToWindrose, convertToWindrose2 } from '../../utils/UploadWindroseCSV';
import { convertSpeedUnits } from '../../utils/CalculationFunctions';
import ReactECharts from 'echarts-for-react';

interface WindroseInputProps {
  windrose?: Windrose;
  setWindrose: (wr: Windrose) => void;
}

const WindRoseInput: React.FC<WindroseInputProps> = ({ windrose, setWindrose }) => {
  ...
  const handleApiSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const windrose = convertToWindrose2(text, dataCount, dataErrors, frequencySum);
  ...
  const changeSpeedUnit = (newUnit: string) => {
    if (!windrose || windrose.speedUnit === newUnit) return;

    let newSpeedBins: [number, number][] = windrose.speedBins.map(([v1, v2]) => [
      Math.round(convertSpeedUnits(v1, windrose.speedUnit, newUnit as SpeedUnits) * 100) / 100,
      isNaN(v2) ? NaN : Math.round(convertSpeedUnits(v2, windrose.speedUnit, newUnit as SpeedUnits) * 100) / 100,
    ])
  ...
  return (
    <div className="windrose-container">
      ...
      {windrose && (
        <label>
          Einheit:
          <select value={windrose.speedUnit} onChange={(e) => changeSpeedUnit(e.target.value)}>
      ...
      {windrose && windrose !== NullWindrose ? (
        <>
          <h3 id="windrose-title">{windrose.name}</h3>
          <div className="chart-div">
            <ReactECharts option={getWindroseChartOption(windrose)} className="chart-container" />
          </div>
          <p>
...
function getWindroseChartOption(windrose: Windrose) {
  // Labels für Richtungsbereiche
  const directions = windrose.data.map(entry => {
    const [start, end] = entry.directionRange;
    return `${start}°–${end}°`;
  });
  const binCount = windrose.speedBins.length;

  const seriesData = [];

  for (let binIdx = 0; binIdx < binCount; binIdx++) {
    const bin = windrose.speedBins[binIdx];
    const binLabel = `${bin[0]}-${bin[1] === Infinity ? '+' : bin[1]} ${windrose.speedUnit}`;

    // Frequenzen je Richtung für das aktuelle Speed-Bin
    const values = windrose.data.map(entry => entry.frequencies[binIdx] ?? 0);
